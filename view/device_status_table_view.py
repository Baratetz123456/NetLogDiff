...
class DeviceStatusTableView:
    def __init__(self, parent, vm, log_cmp_res):
        ...
        self.create_main_burger_buttons(btn_rframe)
    ...
    def create_main_buttons(self, left_frame, right_frame):
        self.collect_logs_btn = ttk.Button(
            left_frame, text=Const.COLLECT_LOG_BTN, command=self.collect_logs_helper
        )
        self.select_pre_log_btn = ttk.Button(
            right_frame, text=Const.PRE_LOG_FOLDER_BTN, command=self.get_pre_log_path
        )
        self.select_post_log_btn = ttk.Button(
            right_frame, text=Const.POST_LOG_FOLDER_BTN, command=self.get_post_log_path
        )
        self.compare_btn = ttk.Button(
            right_frame, text=Const.COMPARE_BTN, command=self.compare_logs_helper
        )
    ...
    def create_labels(self, left_frame, right_frame):
        self.log_collect_lbl = ttk.Label(
            left_frame, textvariable=self.log_collected_date
        )
        self.log_compared_lbl = ttk.Label(
            right_frame, textvariable=self.log_compared_date
        )
    ...
    def create_table(self, tbl_frame, show_commands: List[str] = None):
        ...
        yscrollbar = ttk.Scrollbar(
            tbl_frame, orient=tk.VERTICAL, command=self.tree.yview
        )
        xscrollbar = ttk.Scrollbar(
            tbl_frame, orient=tk.HORIZONTAL, command=self.tree.xview
        )
    ...
    def create_export_btn(self, exp_frame):
        self.export_btn = ttk.Button(
            exp_frame, text=Const.EXPORT_BTN, command=self.export_comparison_logs
        )
    ...
    def populate_table(self, table_data):
        ...
        for index, (hostname, data) in enumerate(table_data.items()):
            ...
            if "Abnormal" in data:
                self.tree.insert(
                    "", tk.END, iid=str(index), values=data, tags=["abnormal"]
                )
            else:
    ...
    def create_main_burger_buttons(self, frame):
        self.burger_btn = ttk.Menubutton(frame, text="☰", style="Big.TMenubutton")

        self.menu = tk.Menu(frame, tearoff=0)
        self.burger_btn["menu"] = self.menu

        self.menu.add_command(
            label=Const.PRE_LOG_FOLDER_BTN, command=self.get_pre_log_path
        )
        self.menu.add_command(
            label=Const.POST_LOG_FOLDER_BTN, command=self.get_post_log_path
        )
        self.menu.add_command(label=Const.COMPARE_BTN, command=self.compare_logs_helper)

    def show_menu_button(self):
        # Hidde horizontal buttons
        self.compare_btn.pack_forget()
        self.select_pre_log_btn.pack_forget()
        self.select_post_log_btn.pack_forget()

        # Show burger button
        self.burger_btn.pack(side=tk.RIGHT, anchor="ne", padx=(3, 0))

    def hide_menu_button(self):
        if not self.burger_btn.winfo_ismapped():
            return
