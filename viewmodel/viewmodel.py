...
class ViewModel:
    ...
    def import_device_config(self):
        try:
            ...
            self.config_status.set(e)
            return False
        
        return True
        
    def import_runtime_data_files(self):
        self.log_file = LogFileManager()
    ...
    def collect_logs_helper(self):
        ...
        reachable_device_config = self.net_inventory.filter_reachable_devices(self.device_config)
        
        # Collect logs by sending show commands to network devices
        result = self.log_model.collect_logs(self.device_config, reachable_device_config)
        ...
        timestamp = self.log_file.get_timestamp()
        ...
        host_paths = self.log_file.generate_host_paths(hostname_with_logs)
